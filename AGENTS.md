...
This repository is a **CLI URL analyzer**. You already have enough context in this file.
...
→ `./lookup https://example.com/` (or `py -3 lookup https://example.com/` on Windows) then analyze output.

User: "why is mobile LCP slow on example.com?"  
→ run lookup, then explain LCP from the new PageSpeed JSON and log.
